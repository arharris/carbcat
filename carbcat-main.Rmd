...
for(user.input.row.i in 1:nrow(user_input_iterations)) {
  ...
  decay_emissions_profile <- data.table(CO2_tonnes=rep(0,100), CO_tonnes=rep(0,100), CH4_tonnes=rep(0,100), NOx_tonnes=rep(0,100), PMUnder10um_tonnes=rep(0,100), PMUnder2.5um_tonnes=rep(0,100), SO2_tonnes=rep(0,100), VOC_tonnes=rep(0,100), char_tonnes=rep(0,100))
  wildfire_emissions_profile <- data.table(CO2_tonnes=rep(0,100), CO_tonnes=rep(0,100), CH4_tonnes=rep(0,100), NOx_tonnes=rep(0,100), PMUnder10um_tonnes=rep(0,100), PMUnder2.5um_tonnes=rep(0,100), SO2_tonnes=rep(0,100), VOC_tonnes=rep(0,100), char_tonnes=rep(0,100))
  ...
  if(user_inputs[variable=="biomass_collection",value]!="No") {
    ...
    warning("Did you remember to update the equipment emissions? Right now there are dummy variable for SO2, because the equipment variables we had were for SOx and fire only reports for SO2. fuck shit damn hell.")
    #######################################################################################################################################
    #######################################################################################################################################
    ...
    harvest_processing_emissions[,':='(CO2_kg = CO2_kg/1000,
                                        CO_kg = CO_kg/1000,
                                        CH4_kg = CH4_kg/1000,
                                        NOx_kg = NOx_kg/1000,
                                        PMUnder10um_kg = PMUnder10um_kg/1000,
                                        PMUnder2.5um_kg = PMUnder2.5um_kg/1000,
                                        SO2_kg = SO2_kg/1000,
                                        VOC_kg = VOC_kg/1000,
                                        char_kg = char_kg/1000)]
    
    setnames(harvest_processing_emissions,c("CO2_kg", "CO_kg", "CH4_kg", "NOx_kg", "PMUnder10um_kg","PMUnder2.5um_kg", "SO2_kg", "VOC_kg", "char_kg"),c("CO2_tonnes", "CO_tonnes", "CH4_tonnes", "NOx_tonnes", "PMUnder10um_tonnes", "PMUnder2.5um_tonnes", "SO2_tonnes", "VOC_tonnes", "char_tonnes"))
    
    transportation_emissions[,':='(CO2_kg = CO2_kg/1000,
                                    CO_kg = CO_kg/1000,
                                    CH4_kg = CH4_kg/1000,
                                        NOx_kg = NOx_kg/1000,
                                        PMUnder10um_kg = PMUnder10um_kg/1000,
    ...
    setnames(transportation_emissions,c("CO2_kg", "CO_kg", "CH4_kg", "NOx_kg", "PMUnder10um_kg","PMUnder2.5um_kg", "SO2_kg", "VOC_kg", "char_kg"),c("CO2_tonnes", "CO_tonnes", "CH4_tonnes", "NOx_tonnes", "PMUnder10um_tonnes", "PMUnder2.5um_tonnes", "SO2_tonnes", "VOC_tonnes", "char_tonnes"))
    
    power_plant_emissions[,':='(CO2_kg = CO2_kg/1000,
                                    CO_kg = CO_kg/1000,
                                    CH4_kg = CH4_kg/1000,
                                        NOx_kg = NOx_kg/1000,
                                        PMUnder10um_kg = PMUnder10um_kg/1000,
    ...
    setnames(power_plant_emissions,c("CO2_kg", "CO_kg", "CH4_kg", "NOx_kg", "PMUnder10um_kg","PMUnder2.5um_kg", "SO2_kg", "VOC_kg", "char_kg"),c("CO2_tonnes", "CO_tonnes", "CH4_tonnes", "NOx_tonnes", "PMUnder10um_tonnes", "PMUnder2.5um_tonnes", "SO2_tonnes", "VOC_tonnes", "char_tonnes"))
  }
  ...
  for (year.i in 1:24) {
    ...
    if(year.i==1) {
      ...
      # Moving away from century_emissions_profile; 
      # If a burn happened, add to the century emissions profile
      # if(user_inputs[variable=='burn_type',value] != "None") {
    ...
    decay_emissions_profile[year.i,':='(CO2_tonnes = CO2_tonnes + decay_emissions[,CO2_tonnes],
                                        CO_tonnes = CO_tonnes + decay_emissions[,CO_tonnes],
                                        CH4_tonnes = CH4_tonnes + decay_emissions[,CH4_tonnes],
                                        NOx_tonnes = NOx_tonnes + decay_emissions[,NOx_tonnes],
                                        PMUnder10um_tonnes = PMUnder10um_tonnes + decay_emissions[,PMUnder10um_tonnes],
                                        PMUnder2.5um_tonnes = PMUnder2.5um_tonnes + decay_emissions[,PMUnder2.5um_tonnes],
                                        SO2_tonnes = SO2_tonnes + decay_emissions[,SO2_tonnes],
                                        VOC_tonnes = VOC_tonnes + decay_emissions[,VOC_tonnes],
                                        char_tonnes = char_tonnes + decay_emissions[,char_tonnes]
                            )]
    ...
    wildfire_emissions_profile[year.i,':='(CO2_tonnes = CO2_tonnes + wildfire_emissions[,CO2_tonnes],
                                           CO_tonnes = CO_tonnes + wildfire_emissions[,CO_tonnes],
                                           CH4_tonnes = CH4_tonnes + wildfire_emissions[,CH4_tonnes],
                                           NOx_tonnes = NOx_tonnes + wildfire_emissions[,NOx_tonnes],
                                           PMUnder10um_tonnes = PMUnder10um_tonnes + wildfire_emissions[,PMUnder10um_tonnes],
                                           PMUnder2.5um_tonnes = PMUnder2.5um_tonnes + wildfire_emissions[,PMUnder2.5um_tonnes],
                                           SO2_tonnes = SO2_tonnes + wildfire_emissions[,SO2_tonnes],
                                           VOC_tonnes = VOC_tonnes + wildfire_emissions[,VOC_tonnes],
                                           char_tonnes = char_tonnes + wildfire_emissions[,char_tonnes]
                               )]
  ...
  # We need several 100 year emissions profiles: century_emissions_profile (the combined emissions)
  ...
  total_residue_base <- case.data[,sum(Recovered_CWD_tonnesAcre_INITIAL, Recovered_FWD_tonnesAcre_INITIAL, Recovered_Foliage_tonnesAcre_INITIAL, Piled_CWD_tonnesAcre_INITIAL, Piled_FWD_tonnesAcre_INITIAL, Piled_Foliage_tonnesAcre_INITIAL, Scattered_CWD_tonnesAcre_INITIAL, Scattered_FWD_tonnesAcre_INITIAL, Scattered_Foliage_tonnesAcre_INITIAL)] * cell_to_acres
  
  # 2) The total technically recovered residue (Recovered_INITIAL)
  technically_recoverable_residue <- case.data[,sum(Recovered_CWD_tonnesAcre_INITIAL, Recovered_FWD_tonnesAcre_INITIAL, Recovered_Foliage_tonnesAcre_INITIAL)] * cell_to_acres
  
  # 3) The total mass sent to the power plant (mass_to_plant)
  total_mass_to_power_plant <- case.data[,sum(mass_to_plant_tonnesAcre)] * cell_to_acres
  
  # 4) The total energy generated, in MWHe. This is power_plant_production
  # power_plant_production 
  
  # Save data - with harvest
  saveRDS(list(century_emissions_profile,harvest_processing_emissions,transportation_emissions,prescribed_burn_emissions,total_residue_base,technically_recoverable_residue,total_mass_to_power_plant,power_plant_production),file=paste("/Volumes/SharedData/Main Folder - Andy H/CBREC_output_directory/",scenario_ID_num,".rds",sep=""))
  # without harvest
  saveRDS(list(century_emissions_profile,total_residue_base,technically_recoverable_residue),file=paste("/Volumes/SharedData/Main Folder - Andy H/CBREC_output_directory/",scenario_ID_num,".rds",sep=""))
}
```
